...
> Bundle live. Four skills, one Project Knowledge block, all wired. Try "change order for [project]" or "light proposal for [GC]" next time you have one.
>
> The artifacts are plain text. You own them. Edit any time. To add a new GC quirk to change-order-draft, open that SKILL.md and append a row to the GC quirks table. To change the dual-signature pattern, edit the Project Knowledge `SIG_PATTERN` field and re-save.
>
> If you want a v2 with takeoff math, AIA G701 generation, or Spanish foreman signatures. Treat that as a self-build target: extend the skills yourself when the need shows up.
