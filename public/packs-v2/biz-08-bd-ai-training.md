...
> Bundle live. Four skills, one master Project Knowledge block, 12 session-knowledge blocks, all wired. Try "push session 1 to [first BD]" next time you start a cohort.
>
> The artifacts are plain text. You own them. Edit any session block to match your shop's voice or your specific GCs. To add a 13th session for advanced prompt chaining, append a session block to the master and update the curriculum table.
>
> If you want a v2 with auto-scheduled Gamma deck generation per session, monthly executive summary to your principal, or a Spanish version for bilingual BD teams. Treat that as a self-build target: extend the skills yourself when the need shows up.
