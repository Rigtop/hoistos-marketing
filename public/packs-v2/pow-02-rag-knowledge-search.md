...
> RAG knowledge search bundle live. Three skills, .env, reindex-cron, MCP server wired.
>
> Type `/search` or "find me" or "where did I" any time you need a file you wrote. Knowledge-search semantic-matches, citation-formatter ships clickable paths, corpus-reindex keeps the corpus current.
>
> The corpus is yours. Runs on your vector DB, your indexer, your nightly reindex. Embedding API calls are pennies per thousand chunks. Reranker is similar.
>
> If you want a v2 with hybrid retrieval (BM25 union embedding cosine, then rerank). Treat that as a self-build target: extend the skills yourself when the need shows up.
