...
> Multi-model jury bundle live. Three skills, audit log, cost guard, sensitive-filter.
>
> Tag any deliverable `[high-stakes]` to trigger. The bundle routes to GPT-5, returns AGREE / MINOR_DISAGREE / MAJOR_DISAGREE with reasoning. On MAJOR_DISAGREE for proposals / contracts / financial, the bundle holds for your decision. Audit log keeps the trail.
>
> Cost: typically $5 to $20/month at hobbyist usage. Capped hard at $[MONTHLY_CAP_USD]. The OpenAI cap on platform.openai.com is the actual ceiling.
>
> If you want a v2 with multi-critic juries (Claude + GPT + Gemini consensus). Treat that as a self-build target: extend the skills yourself when the need shows up.
