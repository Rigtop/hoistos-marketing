...
>
> Drop a contract next time you get one. The parser flags risks in plain English with page numbers and paste-ready redlines. The state-statute-flagger catches jurisdiction-specific issues. The redline-drafter ships the Word language.
>
> Triage, not legal advice. Your lawyer still signs off. The bundle makes sure your hour with the lawyer is spent on the 3 hard issues, not the 9 standard ones.
>
> If you want a v2 with track-changes export (.docx generation). Treat that as a self-build target: extend the skills yourself when the need shows up.
